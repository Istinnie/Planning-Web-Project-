...
function& pGestionTache_AsciiVersHexa(&$ch)
{
	...
	$_PHP_VAR_TMP_50=VersPrimitif(Taille(VersPrimitif($ch)));
	$_PHP_VAR_TMP_51=(isset($nCar) && is_object($nCar)) ? _clone($nCar): null;
	for ($_PHP_VAR_TMP_52=$nCar=&getRef(SetValeur($nCar,1));(1>0&&$_PHP_VAR_TMP_52<=$_PHP_VAR_TMP_50)||(1<0&&$_PHP_VAR_TMP_52>=$_PHP_VAR_TMP_50);$_PHP_VAR_TMP_52+=1,$nCar=$_PHP_VAR_TMP_52)
	{
	...
	if (null!==$_PHP_VAR_TMP_51) { Operateur(95,$_PHP_VAR_TMP_51,$nCar); $nCar =& $_PHP_VAR_TMP_51; }
...
function& pGestionTache_HexaVersAscii(&$sCh)
{
	...
	$_PHP_VAR_TMP_54=(isset($i) && is_object($i)) ? _clone($i): null;
	for ($_PHP_VAR_TMP_55=$i=&getRef(SetValeur($i,1));(1>0&&$_PHP_VAR_TMP_55<=VersPrimitif($nTaille))||(1<0&&$_PHP_VAR_TMP_55>=VersPrimitif($nTaille));$_PHP_VAR_TMP_55+=1,$i=$_PHP_VAR_TMP_55)
	{
	...
	if (null!==$_PHP_VAR_TMP_54) { Operateur(95,$_PHP_VAR_TMP_54,$i); $i =& $_PHP_VAR_TMP_54; }
	for ($s=&WB_Boucle_Pour_Tout_Chaine($_PHP_VAR_TMP_56,VersPrimitif($sChaineEntier),getRef(null),getRef(null),'	',DepuisDebut);$_PHP_VAR_TMP_56->bContinuer();$_PHP_VAR_TMP_56->continuer())
	{
	Operateur(28777,$sChaineAscii,getRef(Caract(VersPrimitif(getRef(Val(VersPrimitif($s)))))));
	}unset($_PHP_VAR_TMP_56);
